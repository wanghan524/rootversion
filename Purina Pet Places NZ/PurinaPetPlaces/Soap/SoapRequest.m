...
@synthesize handler, url, soapAction, postData, receivedData, username, password, deserializeTo, action, logging, defaultHandler, delegate;
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    filesize = (float)response.expectedContentLength;
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)value {
    ...
    //NSLog(@"Data: %@ | %i", [connection.currentRequest.allHTTPHeaderFields objectForKey:@"SOAPAction"], value.length);
    //NSNumber *resourceLength = [NSNumber numberWithUnsignedInteger:[self.receivedData length]];
    [self.delegate sendDownloadProgress:value.length];
}
